        ...
        <div class="form-group {!! $errors->has('category_id') ? 'has-error' : '' !!}">
            ...
            <select class="form-control" name="category_id">
                ...
                @foreach($categories as $p)
                    <option value="{{ $p->id }}" {{ isset($pages->category_id) && $pages->category_id == $p->id ? 'selected="selected"':'' }}>
                                {{ $p->title }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="form-group ">
            ...
            @if(isset($pages->image))
                <div class="imagesquare">
                    <img class="img-responsive" width="200px" height="200px" src="{{ asset('uploads/image/articles/'.$pages->image) }}" />
                </div>
            @endif
